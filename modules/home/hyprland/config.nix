{
  ...
  # Render each monitor entry as a Lua hl.monitor() call
  monitorConfig = lib.concatMapStrings (m:
    ''hl.monitor({ output = "${m.output}", mode = "${m.mode}", position = "${m.position}", scale = ${m.scale} })'' + "\n"
  ) extraMonitorSettings;
  # Render host-specific hardware settings as a Lua hl.config() block (empty string if none)
  hardwareConfig = lib.optionalString (extraHardwareSettings != {}) (
    "hl.config({\n"
    + lib.concatStringsSep ",\n" (lib.mapAttrsToList (section: opts:
        "  "
        + section
        + " = { "
        + lib.concatStringsSep ", " (lib.mapAttrsToList (k: v: k + " = " + toString v) opts)
        + " }"
      )
      extraHardwareSettings)
    + "\n})\n"
...
in {
  wayland.windowManager.hyprland.extraConfig = ''
    ...
    -- Generated by Nix — edit modules/home/hyprland/config.nix
    ...
    hl.on("hyprland.start", function()
      ...
      hl.exec_cmd("pypr &")
      hl.exec_cmd("sleep 1.5 && awww img /home/${username}/Pictures/Wallpapers/${defaultWallpaper}")
    end)
    ...
    hl.config({
      general = {
        ...
        layout             = "dwindle",
        col = {
          active_border   = { colors = { "rgba(${accent1}ff)", "rgba(${accent0}ff)" }, angle = 45 },
    ...
    hl.bind("SUPER + CTRL + code:10",  hl.dsp.window.move({ workspace = 1,  silent = true }))
    hl.bind("SUPER + CTRL + code:11",  hl.dsp.window.move({ workspace = 2,  silent = true }))
    hl.bind("SUPER + CTRL + code:12",  hl.dsp.window.move({ workspace = 3,  silent = true }))
    hl.bind("SUPER + CTRL + code:13",  hl.dsp.window.move({ workspace = 4,  silent = true }))
    hl.bind("SUPER + CTRL + code:14",  hl.dsp.window.move({ workspace = 5,  silent = true }))
    hl.bind("SUPER + CTRL + code:15",  hl.dsp.window.move({ workspace = 6,  silent = true }))
    hl.bind("SUPER + CTRL + code:16",  hl.dsp.window.move({ workspace = 7,  silent = true }))
    hl.bind("SUPER + CTRL + code:17",  hl.dsp.window.move({ workspace = 8,  silent = true }))
    hl.bind("SUPER + CTRL + code:18",  hl.dsp.window.move({ workspace = 9,  silent = true }))
    hl.bind("SUPER + CTRL + code:19",  hl.dsp.window.move({ workspace = 10, silent = true }))
